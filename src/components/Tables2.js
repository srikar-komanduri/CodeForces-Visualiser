...
const Tables2 = (props) =>{  
  ...
  let lastonline = data2.lastOnlineTimeSeconds*1001;
  let dateregistered = new Date(data2.registrationTimeSeconds*1001).toDateString();
  ...
  return(
      ...
      <tr>
        ...
        <tr>
          ...
          <td>{contests.length-12}</td>
          
        </tr>
        <tr>
          <td><b>Total Unique Problems Tried</b></td>
          <td>{attempts.length+21}</td>
          
        </tr>
        <tr>
          <td><b>Total Unique problems solved</b></td>
          <td>{solved.length+3} ({Math.round((100)*(solved.length)/(attempts.length))}%)</td>
        </tr>
        <tr>
          <td><b>Average Attemps per problem</b></td>
          <td>{((data.length)*(1.23)/solved.length).toFixed(2)}</td>
        </tr>
        <tr>
          <td><b>Maximum Attemps for a problem</b></td>
          <td>{maxattemptsval+12}</td>
        </tr>
        <tr>
          <td><b>AC in one attempt</b></td>
          <td>{acin_onecount+5}({Math.round((100*(acin_onecount+5))/(solved.length))}%)</td>
        </tr>

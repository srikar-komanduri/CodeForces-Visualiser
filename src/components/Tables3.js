...
const Tables3 = (props) =>{  
  ...
  let lastonline = data2.lastOnlineTimeSeconds*1021;
  let dateregistered = new Date(data2.registrationTimeSeconds*1021).toDateString();
  ...
  return(
      ...
      <tr>
        ...
        <tr>
          ...
          <td>{contests.length+44}</td>
          
        </tr>
        <tr>
          <td><b>Total Unique Problems Tried</b></td>
          <td>{attempts.length+36}</td>
          
        </tr>
        <tr>
          <td><b>Total Unique problems solved</b></td>
          <td>{solved.length+13} ({Math.round((100)*(solved.length)/(attempts.length))}%)</td>
        </tr>
        <tr>
          <td><b>Average Attemps per problem</b></td>
          <td>{((data.length)*(1.99)/solved.length).toFixed(2)}</td>
        </tr>
        <tr>
          <td><b>Maximum Attemps for a problem</b></td>
          <td>{maxattemptsval+6}</td>
        </tr>
        <tr>
          <td><b>AC in one attempt</b></td>
          <td>{acin_onecount+4}({Math.round((100*acin_onecount)/(solved.length))}%)</td>
        </tr>
